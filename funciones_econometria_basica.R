...
exploratorio <- function(data, x, y) {
  ...
  grafico_linea <- grafico_puntos(data, {{ x }}, {{ y }})
  ...
  lista <- list("grafico_puntos" = grafico_linea, "histograma_x" = histograma_x, 
...
grafico_histograma <- function(data, variable) {
  ...
  return (ggplot(data, aes(x = {{ variable }} )) + geom_histogram(binwidth = ancho, color = "black", fill = "white") + 
...
modelar <- function(datos, x, y) {
  ...
  hisograma_residuos <- grafico_residuos(modelo) 
  h_residuos <- hipotesis_residuos_normal(residuals(modelo))
  h_homocedasticidad <- hipotesis_homocedasticidad(modelo)
  lista <- list("modelo" = modelo, residuos = hisograma_residuos,"h_residuos" = h_residuos,
...
grafico_residuos <- function(modelo) {
  ...
  p <- ggplot(modelo, aes(x = residuals(modelo))) + 
    ...
    geom_vline(xintercept = mean(residuals(modelo)), color = "red")
  return(p)
}
...
grafico_homocedasticidad <- function() {
  # escribe tu codigo
  
  return()
}

hipotesis_autocorrelacion <- function() {
  # escribe tu codigo
